...
if __name__ == "__main__":
    ...
    out_file = 'EvidentIRT_{}_{}_{}_{}_{}person_{}item_{}maxperson_{}maxitem_{}maskperc_{}ability_{}_{}_seed{}'.format(
        args.irt_model, 
        args.dataset,
        args.response_dist,
    ...
    if args.irt_model == '1pl':
        model_class = EvidIRT_1PL
    elif args.irt_model == '2pl':
        model_class = EvidIRT_2PL
    elif args.irt_model == '3pl':
        model_class = EvidIRT_3PL
    elif args.irt_model == 'dina':
        model_class = Evid_DINA
    elif args.irt_model == 'neural':
        model_class = Evid_NCD
    else:
    ...
    def test_acc(epoch, eval_np, name='Test'):
        ...
        ability = infer_dict['ability_gamma'].to(device)
    ...
    def get_infer_dict(loader):
        ...
        with torch.no_grad(): 
            ability_gammas, ability_vs, ability_alphas, ability_betas = [], [], [], []
            item_feat_mus, item_feat_logvars = [], []
            ...
            for _, response, _, mask in loader:
                ...
                _, ability_gamma, ability_v, ability_alpha, ability_beta, _, item_feat_mu, item_feat_logvar = \
                    model.encode(response, mask)

                ability_gammas.append(ability_gamma.cpu())
                ability_vs.append(ability_v.cpu())
                ability_alphas.append(ability_alpha.cpu())
                ability_betas.append(ability_beta.cpu())
            ...
            ability_gammas = torch.cat(ability_gammas, dim=0)
            ability_vs = torch.cat(ability_vs, dim=0)
            ability_alphas = torch.cat(ability_alphas, dim=0)
            ability_betas = torch.cat(ability_betas, dim=0)

            # pbar.close()

        infer_dict['ability_gamma'] = ability_gammas
        infer_dict['ability_v'] = ability_vs
        infer_dict['ability_alpha'] = ability_alphas
        infer_dict['ability_beta'] = ability_betas
    ...
    method_name = f'evidirt_{args.irt_model}_{args.generative_model}'
