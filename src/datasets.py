...
def load_dataset(dataset_name, train=True, **kwargs):
    ...
    elif dataset_name in ['math1', 'a0910', 'FrcSub']:
        return Allinone(name=dataset_name, train=train, **kwargs)
    else:
        raise Exception(f'Dataset {dataset_name} is not supported.')

def load_dataset_tvt(dataset_name, train=True, **kwargs):
    if dataset_name in ['math1', 'math2', 'a0910', 'FrcSub', 'pisa2015']:
        valid_path = os.path.join(DATA_DIR, dataset_name, 'eval_valid.npy')
        test_path = os.path.join(DATA_DIR, dataset_name, 'eval_test.npy')
        return (
            Allinone(name=dataset_name, train=train, fixed=True, **kwargs),
            np.load(valid_path),
            np.load(test_path),
        )
    else:
...
class Allinone(torch.utils.data.Dataset):
    def __init__(self, name='math1', train_rate=1, train=True, fixed=False, max_num_person=None, max_num_item=None, **kwargs):
        super().__init__()

        cache_file = os.path.join(DATA_DIR, name, 'score_matrix_train.npy' if fixed else 'score_matrix.npy')
        ...
        num_person = response.shape[0]
        num_train  = int(train_rate * num_person)
