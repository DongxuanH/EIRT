...
if __name__ == "__main__":
    ...
    def train(epoch):
        ...
        for batch_idx, (index, response, _, mask) in enumerate(train_loader):
            ...
            loss = F.binary_cross_entropy(response_mu, response.float().clamp(min=0), reduction='none')
            loss = loss * mask
            loss = loss.mean()
            loss.backward()
            optimizer.step()
    ...
    def test_acc(epoch):
        ...
        if args.artificial_missing_perc > 0:
            ...
            print('====> Test Epoch: {} Acc: {:.4f}'.format(epoch, missing_imputation_accuracy))
        return missing_imputation_accuracy
    ...
    for epoch in range(args.epochs):
        ...
        test_accs[epoch] = test_acc(epoch)
        if not args.no_test:
